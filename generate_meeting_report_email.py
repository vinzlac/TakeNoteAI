...
def save_email_report(content: str, output_file: Path, meeting_title: Optional[str] = None):
...
Note: Par défaut, les fichiers sont sauvegardés dans:
  output/meeting_reports/{nom_transcription}/compte_rendu_YYYYMMDD_HHMMSS.html
    ...
    # Générer le prompt
    prompt = generate_email_prompt(transcription_text, args.title)
    
    # Appeler cursor-agent
    report_content = call_cursor_agent(prompt, args.cursor_agent_path)
    ...
    if args.output:
        ...
        # Créer un sous-répertoire basé sur le nom de la transcription (sans extension)
    ...
    # Sauvegarder
    if save_email_report(report_content, output_file, args.title):
        print(f"\n📊 Statistiques:")
        print(f"   - Longueur du compte rendu: {len(report_content):,} caractères")
        print(f"   - Répertoire: {output_file.parent}")
